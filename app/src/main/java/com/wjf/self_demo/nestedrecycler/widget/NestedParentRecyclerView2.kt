...
import com.wjf.self_demo.nestedrecycler.widget.FindTarget.findChildScrollTarget

/** 嵌套滑动的父View  */
class NestedParentRecyclerView2 @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : NestedPublicRecyclerView(context, attrs, defStyleAttr), NestedScrollingParent3, NestedScrollingParent2 {
    ...
    /*override fun onInterceptTouchEvent(e: MotionEvent?): Boolean {
        ...
        when (e?.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                Log.d("wjftc", "onInterceptTouchEvent: ACTION_DOWN $result ")
            }
            MotionEvent.ACTION_MOVE -> {
                Log.d("wjftc", "onInterceptTouchEvent: ACTION_MOVE $result ")
            }
            MotionEvent.ACTION_UP -> {
                Log.d("wjftc", "onInterceptTouchEvent: ACTION_UP $result ")
            }
        }
...
//        Log.d("wjftc", "onStartNestedScroll: $child ${(child as? ViewGroup)?.getChildAt(0)} $target")
        // 垂直方向全面接管
        return axes.and(ViewCompat.SCROLL_AXIS_VERTICAL) != 0
    }

    override fun onNestedScrollAccepted(child: View, target: View, axes: Int, type: Int) {
//        Log.d("wjftc", "onNestedScrollAccepted: ${axes.and(ViewCompat.SCROLL_AXIS_VERTICAL) != 0}")
        mParentHelper.onNestedScrollAccepted(child, target, axes, type)
        startNestedScroll(axes, type)
    }

    override fun onStopNestedScroll(target: View, type: Int) {
        Log.d("wjftc", "onStopNestedScroll: ")
    }

    override fun onNestedScroll(target: View, dxConsumed: Int, dyConsumed: Int, dxUnconsumed: Int, dyUnconsumed: Int, type: Int, consumed: IntArray) {
        Log.d("wjftc", "onNestedScroll: 2")
    }

    override fun onNestedScroll(target: View, dxConsumed: Int, dyConsumed: Int, dxUnconsumed: Int, dyUnconsumed: Int, type: Int) {
//        Log.d("wjftc", "onNestedScroll: ")
    }

    override fun onNestedPreScroll(target: View, dx: Int, dy: Int, consumed: IntArray, type: Int) {
        Log.d("wjftc", "onNestedPreScroll: $dy $target")
        if (dy > 0) {
            // 上推
            if (!isScrollEnd) {
                // 如果parent还没有到底，即child还未吸顶，那么parent先消费
                doScrollConsumed(dx, dy, consumed)
            }
        } else {
            // 下拉
            (target as? RecyclerView)?.let {
                if (isChildScrollTop(it)) {
                    // child已经拉到顶了，那就该parent往下
                    doScrollConsumed(dx, dy, consumed)
                }
            }
        }
...
//                stopScroll()
            ...
            MotionEvent.ACTION_POINTER_UP -> {
                ...
                if (pointerId == mActivePointerId) {
                    ...
                    mLastX = e.getX(newPointerIndex)
                    mActivePointerId = e.getPointerId(newPointerIndex)
                }
            }
            MotionEvent.ACTION_MOVE -> {
                val dx = (mLastX - x).toInt()
                val dy = (mLastY - y).toInt()
                Log.d("wjftc", "move: $dy")
                ...
                if (activePointerIndex == -1) {
                    ...
                    if (isScrollEnd || (dy < 0 && child != null && !isChildScrollTop(child))) {
                        /*val dy = (mLastY - y).toInt()
                        val temp
                        if (dispatchNestedPreScroll(0, if (canScrollVertically) dy else 0, mReusableIntPair, mScrollOffset, ViewCompat.TYPE_TOUCH){
...
//                            val deltaY = (mLastY - y).toInt()
                            mTempConsumed[1] = 0
                            child.doScrollConsumed(dx, dy, mTempConsumed)
    ...
    override fun onInterceptTouchEvent(e: MotionEvent): Boolean {
        val isTouchInContent = (
            mContentView != null &&
                ...
                e.y > mContentView!!.top && e.y < mContentView!!.bottom
            )
        ...
        if (isTouchInChildArea) {
            ...
            Log.d("wjftc", "onInterceptTouchEvent: 不拦截")
            return false
        }
        if (isTouchInContent) {
            return false
    ...
    */
    ...
    override fun onStopNestedScroll(target: View, type: Int) {
        if (NestedCeilingHelper.DEBUG) {
            log("onStopNestedScroll type: $type, scrollState: $scrollState")
        }
